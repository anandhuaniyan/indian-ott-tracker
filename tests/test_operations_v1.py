...
def test_data_health_cursor_and_deduplication(database):
    ...
    assert (
        database.query(OperationState).filter_by(name="data_health").one().cursor == 1
    )
    DataHealthService(database).scan(batch_size=1)
    assert (
        database.query(DataQualityIssue)
        .filter_by(movie_id=1, issue_type="missing_cast", resolved_at=None)
        .count()
        <= 1
    )
...
def test_image_validation_and_whole_database_cursors(database, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        service,
        "recover_movie",
        lambda movie, image_type: {"status": "HEALTHY", "type": image_type},
    )
    monkeypatch.setattr(
        service,
        "recover_person",
        lambda person: {"status": "HEALTHY", "type": "profile"},
    )
    result = service.scan(batch_size=10)
    assert result["movies"] == 2 and result["people"] == 2
    assert (
        database.query(OperationState)
        .filter_by(name="image_health_movies")
        .one()
        .cursor
        == 2
    )
    assert (
        database.query(OperationState)
        .filter_by(name="image_health_people")
        .one()
        .cursor
        == 2
    )
...
def test_ott_confidence_canonical_conflict_and_backoff(database, monkeypatch):
    ...
    weak = service.record_evidence(
        2,
        platform="BlogStream",
        source_url="https://blog.invalid/item",
        confidence=35,
        source_rank="unknown",
    )
    assert (
        weak.status == "POSSIBLE"
        and database.query(OttAvailability)
        .filter_by(movie_id=2, provider="BlogStream")
        .first()
        is None
    )
    ...
    assert canonical.status == "released"
    ...
    assert service.next_check_for("FAILED", attempts=4) > service.next_check_for(
        "QUEUED"
    )
...
def test_notifications_continue_after_channel_failure_and_dedupe(database, monkeypatch):
    ...
    monkeypatch.setattr(
        service, "_discord", lambda message: (_ for _ in ()).throw(RuntimeError("down"))
    )
    ...
    assert (
        service.notify("Important worker failure", "high", "worker:test", 60) is False
    )
...
def test_rate_limit_rejects_after_fixed_window(monkeypatch):
    ...
    fake = FakeRedis()
    monkeypatch.setattr(
        "app.core.rate_limit.redis.from_url", lambda *args, **kwargs: fake
    )
    request = Request(
        {
            "type": "http",
...
def test_identity_rate_limit_is_private_and_request_maintenance_is_scheduled(
    monkeypatch,
):
    keys = []
    ...
    monkeypatch.setattr(
        "app.core.rate_limit.redis.from_url", lambda *args, **kwargs: FakeRedis()
    )
    request = Request(
        {
            "type": "http",
    ...
    assert (
        celery_app.conf.beat_schedule["imdb-id-recovery"]["task"]
        == "ratings.imdb_id_backfill"
    )
    assert (
        celery_app.conf.beat_schedule["imdb-rating-refresh"]["task"]
        == "ratings.imdb_refresh"
    )
