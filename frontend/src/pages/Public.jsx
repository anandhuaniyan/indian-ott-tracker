...
export function Person() {
  ...
  if (error) return <Failure error={error}/>; if (!data) return <Loading/>;
  const personLd = { "@context": "https://schema.org", "@type": "Person", name: data.name, ...(data.profile_path && { image: imageUrl(data.profile_path, "original") }), ...(data.department && { jobTitle: data.department }), ...(data.birthday && { birthDate: data.birthday }), ...(data.place_of_birth && { birthPlace: data.place_of_birth }), ...(data.imdb_url && { sameAs: [data.imdb_url] }) };
